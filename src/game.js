class Game {
    ...
    collectSquares(player, event) {
        this.counter++;
        this.gameBoard.filter(square => {
            if (square === event.target.id) {
                player.selections.push(square)
            };
        });
    }

    updateGameBoard(event) {
        this.gameBoard.map(square => {
            if (square === event.target.id) {
                let i = this.gameBoard.indexOf(square);
